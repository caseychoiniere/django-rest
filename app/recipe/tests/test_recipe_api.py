...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_partial_recipe_update(self):
        """test updatting recipe with patch"""
        recipe = sample_recipe(user=self.user)
        recipe.tags.add(sample_tag(user=self.user))
...
class RecipeImageUploadTests(TestCase):
    ...
    def test_upload_image_bad_request(self):
        ...
        url = image_upload_url(self.recipe.id)
        res = self.client.post(url, {'image': 'this is a string'}, format='multipart')

        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)

    def test_filter_recipes_by_tag(self):
        """Test returning recipes by tags"""
        recipe1 = sample_recipe(user=self.user, title='Curry')
        recipe2 = sample_recipe(user=self.user, title='Tagine')
